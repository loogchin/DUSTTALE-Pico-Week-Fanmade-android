...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'snowdin_cave':
			{
				...
				blackgra = new FlxSprite(-100, 0).makeGraphic(20, 20);
				add(blackgra);
			}
	...
	*/
			...
			if(boyfriend.curCharacter == 'pico')
				{
					...
					iconP1 = new HealthIcon('picokr', false);
					iconP1.y = healthBar.y - (iconP1.height / 3.5);
					iconP1.flipX = true;
					iconP1.cameras = [camHUD];
					grpIcons.add(iconP1);
		...
		new FlxTimer().start(0.0001, function(swagTimer:FlxTimer)
			{
				...
				} else
				{
					...
					else if (boyfriend.curCharacter == 'pico') {
						iconP1Prefix = 'pico';
						grpIcons.remove(iconP1);
						iconP1 = new HealthIcon('pico', false);
						iconP1.y = healthBar.y - (iconP1.height / 3.5);
						iconP1.flipX = true;
						iconP1.color = FlxColor.WHITE;
						 iconP1.cameras = [camHUD];
						  grpIcons.add(iconP1);
						healthBar.createFilledBar(0xFF999999, 0xFF66CC66);
					}
					else
				    {
				    		iconP1Prefix = 'bf';
	...
	override function stepHit()
	{
		...
		if (curSong.toLowerCase() == 'dusty-blizzard')
			{
				switch(curStep)
				{
					case 5:
						FlxTween.tween(FlxG.camera, {zoom: 1}, 20, {ease: FlxEase.quadInOut});
					case 193:
						FlxTween.tween(FlxG.camera, {zoom: 0.65}, 0.2, {ease: FlxEase.quadInOut});
						FlxG.camera.flash(FlxColor.BLUE, 1);
						FlxTween.tween(blackgra, {alpha: 1}, 0.5, {ease: FlxEase.quadInOut});
					case 256:
						FlxTween.tween(blackgra, {alpha: 1}, 0.5, {ease: FlxEase.quadInOut});
					case 240:
						FlxTween.tween(blackgra, {alpha: 0}, 0.5, {ease: FlxEase.quadInOut});
					case 257:
